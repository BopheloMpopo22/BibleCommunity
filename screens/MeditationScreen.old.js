...
const MeditationScreen = ({ navigation }) => {
  ...
  const [allCategories, setAllCategories] = useState(scriptureTopics); // Combined default + user topics
  ...
  const loadUserMeditations = async () => {
    try {
      ...
      if (meditations.length > 0) {
        ...
        // Combine with default topics - NEWEST USER TOPICS FIRST
        // Sort: newest user topics first, then default topics
        setAllCategories([...convertedMeditations, ...scriptureTopics]);
      } else {
      ...
      try {
        ...
        if (meditationsJson) {
          ...
          setUserMeditations([]);
          setAllCategories(scriptureTopics);
        }
      } catch (fallbackError) {
  ...
  // OLD CODE REMOVED - Music, voice, timer functionality removed
  const oldHandleCategorySelect = async (category) => {
    // This function is no longer used - replaced with simple navigation
    if (false) {
      ...
      >
        <View style={styles.colorLayer}>
          ...
          >
            ...
            <Text style={styles.createMeditationButtonText}>Create Slideshow</Text>
          </TouchableOpacity>
        </ScrollView>
        </View>
      </ImageBackground>
