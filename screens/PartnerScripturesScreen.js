  ...
  const loadScriptures = async () => {
    try {
      ...
      if (currentUser) {
        ...
        // Filter scriptures: admin sees all, regular partners see only their own
        const userScriptures = isAdmin 
          ? allScriptures  // Admin sees all scriptures
          : allScriptures.filter((scripture) => scripture.authorId === currentUser.uid);  // Partners see only their own
